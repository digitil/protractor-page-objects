...
Component.prototype.count = function() {
    var parent = this.getParent();
    if (parent instanceof Component) {
        return parent.element().element.all(this.getLocator()).count();
    }
...
Component.prototype.all = function(method, then) {
    ...
    if (parent instanceof Component) {
        all = parent.element().element.all(this.getLocator());
    } else {
...
Component.prototype.nth = function(number) {
    var parent = this.getParent();
    if (parent instanceof Component) {
        return parent.element().element.all(this.getLocator()).get(number);
    }
